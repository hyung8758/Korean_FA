...
g2p_input="$mecab_output"
applied_overrides="$prono_dir/applied_overrides.txt"
if [[ -n $pronunciation_dictionary ]]; then
  g2p_input="$prono_dir/mecab_with_overrides.txt"
  "$python_executable" -m koreanfa.pronunciation rewrite-japanese-mecab \
    --dictionary "$pronunciation_dictionary" --input "$mecab_output" \
    --output "$g2p_input" --applied "$applied_overrides"
fi
bash "$language_dir/g2p.sh" "$g2p_input" "$prono_dir/raw_pronunciations.txt"
...
if [[ -s $applied_overrides ]] && ! awk 'NR == FNR { wanted[$1] = 1; next }
  { available[$1] = 1 }
  END { for (word in wanted) if (!(word in available)) { print word; exit 1 } }' \
  "$applied_overrides" "$prono_dir/valid_lexicon.txt"; then
  echo "A custom Japanese pronunciation could not be converted to the model phone inventory." >&2
  exit 1
fi
...
tr ' ' '\n' < "$g2p_input" | awk -F'+' '{print $1}' > "$prono_dir/mecab_words.txt"
