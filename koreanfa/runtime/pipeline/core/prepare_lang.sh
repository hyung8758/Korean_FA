...
. runtime/pipeline/core/parse_options.sh

if [ $# -ne 4 ]; then
  echo "usage: runtime/pipeline/core/prepare_lang.sh <dict-src-dir> <oov-dict-entry> <tmp-dir> <lang-dir>"
  echo "e.g.: runtime/pipeline/core/prepare_lang.sh data/local/dict <SPOKEN_NOISE> data/local/lang data/lang"
...
! runtime/pipeline/core/validate_dict_dir.pl $srcdir && \
...
if ! runtime/pipeline/core/validate_dict_dir.pl $srcdir >&/dev/null; then
  runtime/pipeline/core/validate_dict_dir.pl $srcdir  # show the output.
...
if $share_silence_phones; then
  ...
  cat $srcdir/silence_phones.txt | awk '{printf("%s ", $0); } END{printf("\n");}' | cat - $srcdir/nonsilence_phones.txt | \
    runtime/pipeline/core/apply_map.pl $tmpdir/phone_map.txt > $dir/phones/sets.txt
  ...
  cat $srcdir/{,non}silence_phones.txt | runtime/pipeline/core/apply_map.pl $tmpdir/phone_map.txt > $dir/phones/sets.txt
  cat $dir/phones/sets.txt | awk '{print "shared", "split", $0;}' > $dir/phones/roots.txt
fi

cat $srcdir/silence_phones.txt | runtime/pipeline/core/apply_map.pl $tmpdir/phone_map.txt | \
  awk '{for(n=1;n<=NF;n++) print $n;}' > $dir/phones/silence.txt
cat $srcdir/nonsilence_phones.txt | runtime/pipeline/core/apply_map.pl $tmpdir/phone_map.txt | \
...
cat $srcdir/extra_questions.txt 2>/dev/null | runtime/pipeline/core/apply_map.pl $tmpdir/phone_map.txt \
...
if "$silprob"; then
  ndisambig=`runtime/pipeline/core/add_lex_disambig.pl --pron-probs --sil-probs $tmpdir/lexiconp_silprob.txt $tmpdir/lexiconp_silprob_disambig.txt`
else
  ndisambig=`runtime/pipeline/core/add_lex_disambig.pl --pron-probs $tmpdir/lexiconp.txt $tmpdir/lexiconp_disambig.txt`
fi
...
cat $dir/phones/align_lexicon.txt | runtime/pipeline/core/sym2int.pl -f 3- $dir/phones.txt | \
  runtime/pipeline/core/sym2int.pl -f 1-2 $dir/words.txt > $dir/phones/align_lexicon.int
...
if $silprob; then
  ...
  runtime/pipeline/core/make_lexicon_fst_silprob.pl $tmpdir/lexiconp_silprob_disambig.txt $srcdir/silprob.txt $silphone '#'$ndisambig | \
     sed 's=\#[0-9][0-9]*=<eps>=g' | \
     fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
     --keep_isymbols=false --keep_osymbols=false |   \
     fstarcsort --sort_type=olabel > $dir/L.fst || exit 1;
else
  runtime/pipeline/core/make_lexicon_fst.pl --pron-probs $tmpdir/lexiconp.txt $sil_prob $silphone | \
    fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
    --keep_isymbols=false --keep_osymbols=false | \
     fstarcsort --sort_type=olabel > $dir/L.fst || exit 1;
fi
...
cat $dir/oov.txt | runtime/pipeline/core/sym2int.pl $dir/words.txt >$dir/oov.int || exit 1;
...
runtime/pipeline/core/sym2int.pl $dir/phones.txt <$dir/phones/wdisambig.txt >$dir/phones/wdisambig_phones.int
runtime/pipeline/core/sym2int.pl $dir/words.txt <$dir/phones/wdisambig.txt >$dir/phones/wdisambig_words.int
...
for f in silence nonsilence optional_silence disambig context_indep; do
  runtime/pipeline/core/sym2int.pl $dir/phones.txt <$dir/phones/$f.txt >$dir/phones/$f.int
  runtime/pipeline/core/sym2int.pl $dir/phones.txt <$dir/phones/$f.txt | \
   awk '{printf(":%d", $1);} END{printf "\n"}' | sed s/:// > $dir/phones/$f.csl || exit 1;
done

for x in sets extra_questions; do
  runtime/pipeline/core/sym2int.pl $dir/phones.txt <$dir/phones/$x.txt > $dir/phones/$x.int || exit 1;
done

runtime/pipeline/core/sym2int.pl -f 3- $dir/phones.txt <$dir/phones/roots.txt \
   > $dir/phones/roots.int || exit 1;

#if $position_dependent_phones; then
if [ -f $dir/phones/word_boundary.txt ]; then
  runtime/pipeline/core/sym2int.pl -f 1 $dir/phones.txt <$dir/phones/word_boundary.txt \
    > $dir/phones/word_boundary.int || exit 1;
fi

silphonelist=`cat $dir/phones/silence.csl`
nonsilphonelist=`cat $dir/phones/nonsilence.csl`
runtime/pipeline/core/gen_topo.pl $num_nonsil_states $num_sil_states $nonsilphonelist $silphonelist >$dir/topo
...
if $silprob; then
  ...
  runtime/pipeline/core/make_lexicon_fst.pl --pron-probs $tmpdir/lexiconp_disambig.txt $sil_prob $silphone '#'$ndisambig | \
     fstcompile --isymbols=$dir/phones.txt --osymbols=$dir/words.txt \
     --keep_isymbols=false --keep_osymbols=false | \
     fstaddselfloops  $dir/phones/wdisambig_phones.int $dir/phones/wdisambig_words.int | \
     fstarcsort --sort_type=olabel > $dir/L_disambig.fst || exit 1;
...
! runtime/pipeline/core/validate_lang.pl $dir && echo "$(basename $0): error validating output" &&  exit 1;
