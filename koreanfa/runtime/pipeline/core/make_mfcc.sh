...
nj=4
cmd=run.pl
mfcc_config=runtime/config/mfcc.conf
compress=true
...
. runtime/pipeline/core/parse_options.sh || exit 1;
...
if [ $# != 3 ]; then
   ...
   echo "  --cmd (runtime/pipeline/core/run.pl|runtime/pipeline/core/queue.pl <queue opts>) # how to run jobs."
   exit 1;
fi
...
runtime/pipeline/core/validate_data_dir.sh --no-text --no-feats $data || exit 1;
...
for n in $(seq $nj); do
  ...
  # runtime/pipeline/core/create_data_link.pl for more info.
  runtime/pipeline/core/create_data_link.pl $mfccdir/raw_mfcc_$name.$n.ark
done
...
if [ -f $data/segments ]; then
  ...
  runtime/pipeline/core/split_scp.pl $data/segments $split_segments || exit 1;
  ...
  runtime/pipeline/core/split_scp.pl $scp $split_scps || exit 1;
...
if [ $nf -ne $nu ]; then
  ...
  echo "consider using runtime/pipeline/core/fix_data_dir.sh $data"
fi
