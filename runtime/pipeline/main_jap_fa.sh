...
# Code directory
code_dir=runtime/pipeline/core
# conf directory
conf_dir=runtime/config
# log directory
...
# Input is normalized to mono 16 kHz PCM WAV by koreanfa.audio.

python runtime/pipeline/fa_prep_data.py $data_dir $trans_dir >> $log_dir/process.$log_name.log || exit 1
...
bash runtime/pipeline/run_japanese_mecab.sh $raw_sent_dir/$txt_rename $raw_sent_dir/mecab_reuslt.txt
bash runtime/pipeline/run_japanese_g2p.sh $raw_sent_dir/mecab_reuslt.txt $prono_dir/raw_prono_list
...
bash runtime/pipeline/prepare_jap_new_lang.sh $dict_dir $lang_dir "<unk>" &>/dev/null
...
if [ $model_type == "gmm" ]; then
	$code_dir/make_mfcc.sh --nj $mfcc_nj --cmd "$cmd" --mfcc-config runtime/config/mfcc.conf $trans_dir $log_dir $trans_dir/$mfccdir >> $log_dir/process.$log_name.log
	...
	$code_dir/make_mfcc.sh --nj $mfcc_nj --cmd "$cmd" --mfcc-config runtime/config/mfcc_hires.conf $trans_dir $log_dir $trans_dir/$mfcchiresdir >> $log_dir/process.$log_name.log
	$code_dir/fix_data_dir.sh $trans_dir >> $log_dir/process.$log_name.log
...
if [ $passing -ne 1 ]; then
	...
	python runtime/pipeline/fix_ctm_float.py $ali_dir/raw_ali.ctm $ali_dir/fixed_ali.ctm
	...
	python runtime/pipeline/id2phone.py $result_dir/phones.txt \
								   $result_dir/segments \
	...
	python runtime/pipeline/generate_textgrid.py $tg_word_opt $tg_phone_opt \
							$result_dir/tmp_fa \
