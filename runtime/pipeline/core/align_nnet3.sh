...
extra_files="$data/ivector_online.scp $data/ivector_period"
...
check_phones_compatible.sh $lang/phones.txt $srcdir/phones.txt || exit 1;
...
ivector_period=$(cat $data/ivector_period) || exit 1;
...
if [ ! -z "$graphs_scp" ]; then
  ...
  tra="scp:runtime/pipeline/core/filter_scp.pl $sdata/JOB/utt2spk $graphs_scp |"
