...
function filter_file {
  ...
  runtime/pipeline/core/filter_scp.pl $filter ${file_to_filter}.tmp > $file_to_filter
...
function filter_recordings {
  ...
  # recording-ids.
    ...
    runtime/pipeline/core/filter_scp.pl $data/wav.scp $tmpdir/recordings > $tmpdir/recordings.tmp
...
function filter_speakers {
  ...
  runtime/pipeline/core/utt2spk_to_spk2utt.pl $data/utt2spk > $data/spk2utt
  ...
  runtime/pipeline/core/spk2utt_to_utt2spk.pl $data/spk2utt > $data/utt2spk
...
function filter_utts {
  ...
  for x in feats.scp text segments utt2lang $maybe_wav; do
    if [ -f $data/$x ]; then
      runtime/pipeline/core/filter_scp.pl $data/$x $tmpdir/utts > $tmpdir/utts.tmp
  ...
  for x in utt2spk utt2uniq feats.scp vad.scp text segments utt2lang utt2dur $maybe_wav; do
    if [ -f $data/$x ]; then
      cp $data/$x $data/.backup/$x
      if ! cmp -s $data/$x <( runtime/pipeline/core/filter_scp.pl $tmpdir/utts $data/$x ) ; then
        runtime/pipeline/core/filter_scp.pl $tmpdir/utts $data/.backup/$x > $data/$x
      fi
    fi
  done
