...
These dependencies are distinct from the Japanese MeCab and IPADIC resources contained in the managed Linux engine below.

## Linux engine (`koreanfa engine install`)

The separately downloaded engine archive is a different distributable from the Python wheel. It includes complete, version-matched notice text in its `licenses/` directory:
